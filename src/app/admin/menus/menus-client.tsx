  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold">{dict.admin.menus || '菜单管理'}</h1>
          <p className="text-xs text-muted-foreground">管理前台导航菜单及其排序</p>
        </div>
                ...
                <tr key={menu.id} className="hover:bg-accent/30 group">
                  <td className="px-4 py-2">
                    <div className="flex flex-col">
                       <button onClick={() => moveUp(index)} className="hover:text-primary">▲</button>
                       <button onClick={() => moveDown(index)} className="hover:text-primary">▼</button>
                    </div>
                  </td>
                  <td className="px-4 py-2">
                    <input
                      type="text"
                      value={menu.label}
                      onChange={(e) => handleChange(menu.id, 'label', e.target.value)}
                  ...
                  <td className="px-4 py-2">
                    <select
                      value={menu.target}
                      onChange={(e) => handleChange(menu.id, 'target', e.target.value)}
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      onChange={(e) => handleChange(menu.id, 'isActive', e.target.checked)}
                      className="w-4 h-4"
                    />
                  </td>
