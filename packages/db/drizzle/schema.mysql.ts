import { mysqlTable, text, serial, int, boolean, timestamp, json, varchar, index } from 'drizzle-orm/mysql-core'
...
export const postSnippets = mysqlTable('post_snippets', {
  postId: int('post_id').notNull().references(() => posts.id, { onDelete: 'cascade' }),
  snippetId: int('snippet_id').notNull().references(() => snippets.id, { onDelete: 'cascade' }),
})
...
export const comments = mysqlTable('comments', {
  ...
  parentId: int('parent_id').references((): any => comments.id, { onDelete: 'cascade' }),
